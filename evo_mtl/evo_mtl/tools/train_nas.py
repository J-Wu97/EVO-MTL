...
from apex import amp #英伟达提供的混合精度训练工具，在不降低性能的情况下，将模型训练提升2-4倍，训练显存消耗减少为原来一半。
...
def main():
    ...
    # Preparing for DDP training 分布式训练准备
    ...
    cfg.merge_from_file(args.config_file)#传入文件路径
    cfg.EXPERIMENT_NAME = args.config_file.split('/')[-1][:-5]#分割出实验名称
    ...
    split = int(np.floor(cfg.ARCH.TRAIN_SPLIT * num_train))#训练数据的划分
