...
class SingleTaskNet(nn.Module):
    def __init__(self, cfg, net1, net2):
        ...
        self.task1, self.task2 = get_tasks(cfg)#对应的是哪两个任务
    ...
    def loss(self, x, labels):
        ...
        result.loss = result.loss1 + self.cfg.TRAIN.TASK2_FACTOR * result.loss2#这个损失函数不是等比例的
        return result
    '''
    forward函数，定义前向传播函数
    '''
...
class SharedFeatureNet(nn.Module):
    ...
    def forward(self, x):
        x = self.net1.base(x)
        for stage_id in range(self.num_stages):
            x = self.net1.stages[stage_id](x)#task1 and task2 共享一个主干网络，最后对应不同的输出
